...
from src.agents.market_watch_dog_agent import market_watch_dog_agent as graph
# from src.bot.orchestrator_graph import graph
...
@router.post("/trade-agent")
async def trade_agent(request: Request):
    """Endpoint for running the agent."""
    try:
        body = await request.json()
    except Exception as e:
        from json import JSONDecodeError
        if isinstance(e, JSONDecodeError) or 'Expecting value' in str(e):
...
async def history(thread_id: str):
    ...
    # Fetch state history using graph.aget_state_history
    records = []
    ...
    memory = get_memory(thread_id)
    try:
        # Provide a dummy input key and value to satisfy the requirement of one input
        # The actual content of this input might need to be adjusted based on how memory is intended to be used here.
        conversation_history = memory.load_memory_variables({"input": ""}).get("chat_history", [])
        ...
        print(f"Error loading history for thread_id {thread_id}: {e}")
